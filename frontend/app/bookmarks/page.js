...
export default function BookmarksPage() {
  ...
  const resetForm = () =>
    setForm({ id: null, url: "", title: "", description: "", tags: [] });
  ...
  const onSubmit = async (e) => {
    ...
    if (!trimmedUrl) {
      ...
      try {
        ...
        nextErrors.url = "Please enter a valid URL (including http/https)";
      }
    }
    ...
    try {
      ...
      if (form.id) {
        await api.updateBookmark(form.id, payload);
      } else {
  ...
  const toggleFavorite = async (bookmark) => {
    try {
      setLoading(true);
      setError("");
      await api.updateBookmark(bookmark._id, { favorite: !bookmark.favorite });
      await loadBookmarks();
    } catch (err) {
  ...
  const visibleBookmarks = showFavoritesOnly
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB]">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
          ...
          {visibleBookmarks.map((b) => (
            ...
            >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mb-2 text-sm font-medium text-[#14B8A6] hover:underline"
              >
                {b.url}
              </a>
              {b.description && (
        ...
        {visibleBookmarks.length === 0 && !loading && (
          <div className="py-16 text-center">
            ...
            <p className="mt-2 text-sm text-[#6B7280]">
              Use the &quot;+ Add Bookmark&quot; button to save your first link.
            </p>
          </div>
        )}
